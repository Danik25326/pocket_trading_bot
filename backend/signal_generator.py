        ...
        try:
            ...
            if last_update:
                try:
                    ...
                    logger.info(f"⏳ Остання генерація була {minutes_diff:.1f} хвилин тому")
                    logger.info(f"   Останнє оновлення: {last_time.strftime('%H:%M:%S')} UTC")
                    
                    # ДОЗВОЛЯЄМО ЗАПУСК КОЖНІ 9,9 ХВИЛИН
                    if minutes_diff < 9.9:
                        logger.info(f"⚠️  Ще не пройшло достатньо часу для наступної генерації.")
                        logger.info(f"   Наступний автоматичний запуск через {10 - minutes_diff:.1f} хвилин")
                        
                        # Автоматичне очищення старих сигналів все одно виконуємо
...
async def main():
    ...
    # Перевірка, чи не було запуску менше 9,9 хвилин тому
    ...
    if last_update:
        try:
            ...
            # ДОЗВОЛЯЄМО ЗАПУСК КОЖНІ 9,9 ХВИЛИН ЗАМІСТЬ 9
            if minutes_diff < 9.9:
